  ...
  const formatAndSetCart = (dbCart: any) => {
    ...
    const formattedItems = dbCart.items.map((item: any) => {
      ...
      return {
        ...
        price: item.price || product.price || 0, // IMPORTANT: Use item price because bulk pricing is handled on server!
        imageSrc: product.images?.[0]?.secure_url || '',
        quantity: item.quantity,
        variantId: item.variantId,
        sellingModeInfo: item.sellingModeInfo
      };
    });
