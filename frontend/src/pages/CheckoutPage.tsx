...
export default function CheckoutPage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const payloadItems = items.map(i => ({ productId: i.id, quantity: i.quantity, variantId: i.variantId, sellingModeInfo: i.sellingModeInfo }));
    ...
    try {
      if (paymentMethod === 'Razorpay') {
        const payload = {
          ...
          items: items.map(i => ({ product: i.id, quantity: i.quantity, variantId: i.variantId, sellingModeInfo: i.sellingModeInfo })),
          couponCode: coupon,
          notes,
        };
            ...
            <div className="flex flex-col gap-4 mb-8 max-h-[40vh] overflow-y-auto pr-2">
              {items.map((item) => (
                <div key={item.id} className="flex gap-4 items-center">
                  ...
                  <span className="font-sans font-bold text-[15px]">₹{(item.price * item.quantity)}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-3 pt-6 border-t border-gray-100">
              ...
              {discount > 0 && (
                <div className="flex justify-between items-center text-green-600">
                  ...
                  <span className="font-sans font-medium">-₹{discount}</span>
                </div>
              )}
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-100">
                ...
                <span className="font-sans text-[24px] font-bold">₹{grandTotal}</span>
              </div>
            </div>
