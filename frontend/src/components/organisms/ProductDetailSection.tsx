...
export interface ProductDetailSectionProps {
  product: {
    ...
    sellingMode?: 'standard' | 'set' | 'moq';
    setOptions?: { _id?: string; quantity: number; price: number; compareAtPrice?: number }[];
    moqConfig?: { minimumQuantity: number; maximumQuantity?: number; basePrice: number; additionalUnitPrice: number; };
  };
}

export const ProductDetailSection: React.FC<ProductDetailSectionProps> = ({ product }) => {
  const [quantity, setQuantity] = useState(product.sellingMode === 'moq' && product.moqConfig ? product.moqConfig.minimumQuantity : 1);
  const [selectedSetOptionId, setSelectedSetOptionId] = useState<string | null>(
    product.sellingMode === 'set' && product.setOptions && product.setOptions.length > 0
      ? (product.setOptions[0]._id || null)
      : null
  );
  ...
  const basePrice = selectedVariant && !isBaseVariant && selectedVariant.price && selectedVariant.price > 0 ? selectedVariant.price : product.price;
  
  let currentPrice = basePrice;
  let currentComparePrice = product.compareAtPrice || 0;

  if (product.sellingMode === 'set' && product.setOptions && product.setOptions.length > 0) {
    const selectedOpt = product.setOptions.find(opt => opt._id === selectedSetOptionId) || product.setOptions[0];
    if (selectedOpt) {
      currentPrice = selectedOpt.price;
      currentComparePrice = selectedOpt.compareAtPrice || 0;
    }
  } else if (product.sellingMode === 'moq' && product.moqConfig) {
    const minQty = product.moqConfig.minimumQuantity || 1;
    const extraItems = quantity > minQty ? quantity - minQty : 0;
    currentPrice = product.moqConfig.basePrice + (extraItems * product.moqConfig.additionalUnitPrice);
  ...
  const handleAddToCart = () => {
    ...
    if (product.sellingMode === 'set' && product.setOptions) {
      ...
      if (opt) {
        price = opt.price;
        qty = opt.quantity;
        sellingModeInfo = { mode: 'set', setId: opt._id, setQuantity: opt.quantity };
      }
    } else if (product.sellingMode === 'moq' && product.moqConfig) {
      const extraItems = qty - product.moqConfig.minimumQuantity;
      price = product.moqConfig.basePrice + (extraItems > 0 ? extraItems * product.moqConfig.additionalUnitPrice : 0);
      sellingModeInfo = { mode: 'moq', moqQuantity: qty };
    }
    ...
    addToCart({
      ...
      quantity: product.sellingMode === 'set' ? 1 : qty,
      variantId: selectedVariant && !selectedVariant.isBase ? selectedVariant._id : undefined,
      sellingModeInfo
    });
  };
  ...
  const handleBuyNow = () => {
    ...
    sessionStorage.setItem('temp_checkout_session', JSON.stringify([{
      id: selectedVariant && !selectedVariant.isBase ? `${product.id}-${selectedVariant.name.replace(/\s+/g, '-')}` : product.id,
      title: currentTitle,
      price: price,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div className="flex flex-col pt-4">
          ...
          <div className="flex items-center gap-3 mb-6">
            <Typography variant="h5" className="font-sans font-bold text-[#03989E]">₹{Math.round(currentPrice)}</Typography>
            {currentComparePrice > currentPrice && (
              <>
                <span className="text-gray-400 line-through text-sm font-semibold">₹{currentComparePrice}</span>
          ...
          {/* Quantity / Sets / MOQ Options */}
          {product.sellingMode === 'set' && product.setOptions && product.setOptions.length > 0 && (
            <div className="mb-6">
              <h4 className="text-sm font-semibold text-gray-900 mb-3">Select Pack Size</h4>
              <div className="flex flex-wrap gap-3">
                {product.setOptions.map(opt => (
                  <button
                    key={opt._id}
                    onClick={() => setSelectedSetOptionId(opt._id!)}
                    className={`relative flex flex-col items-start px-4 py-3 border-2 rounded-xl transition-all w-full sm:w-auto min-w-[120px] ${
                      selectedSetOptionId === opt._id
                        ? 'border-[#03989E] bg-[#03989E]/5'
                        : 'border-gray-200 hover:border-gray-300'
                    }`}
                  >
                    <span className={`text-sm font-bold ${selectedSetOptionId === opt._id ? 'text-[#03989E]' : 'text-gray-800'}`}>
                      Set of {opt.quantity}
                    </span>
          ...
          {product.sellingMode === 'moq' && product.moqConfig && (
            <div className="mb-6 p-4 rounded-xl bg-orange-50 border border-orange-100">
              <h4 className="text-sm font-semibold text-orange-800 mb-2">Wholesale Rules (MOQ)</h4>
              <ul className="text-sm text-orange-700 space-y-1 list-disc list-inside">
                <li>Minimum Order Quantity: <strong>{product.moqConfig.minimumQuantity} items</strong></li>
                <li>Base price for {product.moqConfig.minimumQuantity} items: <strong>₹{Math.round(product.moqConfig.basePrice)}</strong></li>
                <li>Each additional item: <strong>+₹{Math.round(product.moqConfig.additionalUnitPrice)}/item</strong></li>
                {product.moqConfig.maximumQuantity && <li>Maximum allowed: <strong>{product.moqConfig.maximumQuantity} items</strong></li>}
              </ul>
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden bg-white">
                  <button 
                    onClick={() => setQuantity(Math.max(product.moqConfig!.minimumQuantity, quantity - 1))}
                    className="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition-colors"
                    disabled={quantity <= product.moqConfig.minimumQuantity}
                  >
                    -
                  </button>
                  ...
                  <button 
                    onClick={() => {
                      if (product.moqConfig?.maximumQuantity) {
                        setQuantity(Math.min(product.moqConfig.maximumQuantity, quantity + 1));
                      } else {
                        setQuantity(quantity + 1);
                      }
                    }}
                    className="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition-colors"
                    disabled={!!product.moqConfig.maximumQuantity && quantity >= product.moqConfig.maximumQuantity}
                  >
                    +
                  </button>
                </div>
          ...
          {(!product.sellingMode || product.sellingMode === 'standard') && (
            <div className="flex items-center gap-4 mb-6">
              <span className="font-sans font-bold text-[15px]" style={{ color: 'var(--text-primary)' }}>Quantity</span>
              <div className="flex items-center border rounded-full overflow-hidden shadow-xs" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-page)' }}>
                <button 
                ...
                <span className="w-10 h-10 flex items-center justify-center font-sans font-medium text-[15px]" style={{ color: 'var(--text-primary)' }}>
                  {quantity}
                </span>
                <button 
