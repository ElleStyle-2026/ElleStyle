...
export interface ProductReviewSectionProps {
  ...
  reviews: {
    ...
    images?: string[]; // Fallback for old data
    likes: number;
    dislikes: number;
  }[];
}
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        mediaData = uploadData.data.map((item: any) => ({
          url: item.secure_url,
      ...
      const res = await apiClient(`/api/v1/products/${productId}/reviews`, {
        ...
        body: JSON.stringify({ rating, comment, media: mediaData })
      });
      ...
      if (data.success) {
        ...
        // Optimistically add to list if approved (backend might auto-approve based on settings)
          ...
          <div className="mb-16">
            <form onSubmit={handleSubmitReview} className="p-8 md:p-10 rounded-[2rem] border border-gray-100 bg-white shadow-xl max-w-3xl mx-auto w-full transition-all duration-300 transform origin-top">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-medium text-gray-700">Upload Photos/Videos (Max 3, Video max 1 min)</label>
                  <div className="flex flex-wrap gap-4 mt-2">
                    {files.map((file, i) => (
                      <div key={i} className="relative w-24 h-24 rounded-xl border overflow-hidden bg-gray-50">
                        {file.type.startsWith('video/') ? (
                          <video src={URL.createObjectURL(file)} className="w-full h-full object-cover" />
                        ) : (
                          <img src={URL.createObjectURL(file)} className="w-full h-full object-cover" />
                        )}
                        <button type="button" onClick={() => removeFile(i)} className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs shadow hover:bg-red-600 z-10">
                          ✕
                        </button>
                      </div>
                    ))}
                    {files.length < 3 && (
                      <label className="w-24 h-24 rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:text-black hover:border-black hover:bg-gray-50 transition-colors cursor-pointer">
                        <span className="text-2xl leading-none">+</span>
                        <input type="file" className="hidden" accept="image/*,video/mp4,video/webm,video/quicktime" multiple onChange={handleFileChange} ref={fileInputRef} />
                      </label>
                    )}
                  </div>
                </div>

                <button type="submit" disabled={isUploading} className={`px-8 py-3.5 mt-2 rounded-full bg-black text-white text-[15px] font-sans font-medium self-center hover:bg-black/80 transition-all w-full md:w-auto min-w-[200px] ${isUploading ? 'opacity-70 cursor-not-allowed' : 'hover:scale-105 hover:shadow-lg'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          {reviewsList.map((review) => (
            <div key={review.id} className="rounded-3xl p-6 md:p-8 flex flex-col" style={{ backgroundColor: 'var(--surface-card)', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
              ...
              {/* Text */}
              <p className="font-sans text-[13px] leading-relaxed mb-6 flex-1 whitespace-pre-wrap" style={{ color: 'var(--text-secondary)' }}>
                {review.text}
              </p>
              ...
              {(review.media && review.media.length > 0) ? (
                <div className="flex gap-3 mb-6 overflow-x-auto pb-2">
                  {review.media.map((item, i) => (
                    <div key={i} className="w-24 h-24 shrink-0 rounded-2xl overflow-hidden bg-black/5 relative">
                      {item.type === 'video' ? (
                ...
                // Fallback for older reviews with only images array
                <div className="flex gap-3 mb-6 overflow-x-auto pb-2">
                  {review.images.map((img, i) => (
                    <div key={i} className="w-24 h-24 shrink-0 rounded-2xl overflow-hidden bg-black/5">
