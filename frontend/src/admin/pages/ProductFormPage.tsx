...
export default function ProductFormPage() {
  ...
  const [formData, setFormData] = useState<Partial<AdminProduct>>({
    ...
    setOptions: [],
    moqConfig: { minimumQuantity: 5, maximumQuantity: 100, basePrice: 0, additionalUnitPrice: 0 },
  });
  ...
  const fetchProduct = async (pId: string) => {
    try {
      ...
      setFormData({
        ...
        sellingMode: data.sellingMode || 'standard',
        setOptions: data.setOptions || [],
        moqConfig: data.moqConfig || { minimumQuantity: 5, maximumQuantity: 100, basePrice: 0, additionalUnitPrice: 0 },
      });
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-sm border border-t-0 border-gray-200 rounded-b-lg p-6">
        ...
        {activeTab === 'Pricing' && (
          <div className="space-y-6">
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">Selling Mode</label>
              <div className="flex flex-wrap gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="sellingMode" value="standard" checked={formData.sellingMode === 'standard'} onChange={handleChange} className="text-[#03989E] focus:ring-[#03989E]" />
                  <span className="text-sm">Standard (Single Unit)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="sellingMode" value="set" checked={formData.sellingMode === 'set'} onChange={handleChange} className="text-[#03989E] focus:ring-[#03989E]" />
                  <span className="text-sm">Fixed Sets (Packs)</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="sellingMode" value="moq" checked={formData.sellingMode === 'moq'} onChange={handleChange} className="text-[#03989E] focus:ring-[#03989E]" />
                  <span className="text-sm">Minimum Order Qty (MOQ)</span>
                </label>
              </div>
            </div>

            {(!formData.sellingMode || formData.sellingMode === 'standard') && (
            ...
            {formData.sellingMode === 'set' && (
              <div className="space-y-4 border p-4 rounded-lg bg-gray-50">
                <div className="flex justify-between items-center">
                  <h4 className="text-sm font-semibold text-gray-800">Set Options</h4>
                  <button type="button" onClick={() => setFormData({ ...formData, setOptions: [...(formData.setOptions || []), { quantity: 2, price: 0 }] })} className="text-xs bg-white border border-gray-200 px-3 py-1.5 rounded-md hover:bg-gray-100 flex items-center gap-1"><Plus className="w-3 h-3" /> Add Set</button>
                </div>
                <div className="grid grid-cols-1 gap-4">
                  {(formData.setOptions || []).map((setOpt, idx) => (
                    <div key={idx} className="flex flex-wrap sm:flex-nowrap gap-4 items-end bg-white p-3 rounded border">
                      <div className="flex-1 min-w-[120px]">
                        <label className="block text-xs text-gray-500 mb-1">Set Size (Qty)</label>
                        <input type="number" value={setOpt.quantity} onChange={(e) => {
                          const newSets = [...(formData.setOptions || [])];
                          newSets[idx].quantity = Number(e.target.value);
                          setFormData({ ...formData, setOptions: newSets });
                        }} className="w-full text-sm border border-gray-300 rounded px-2 py-1.5" />
                      </div>
                      <div className="flex-1 min-w-[120px]">
                        <label className="block text-xs text-gray-500 mb-1">Set Price (₹)</label>
                        <input type="number" value={setOpt.price} onChange={(e) => {
                          const newSets = [...(formData.setOptions || [])];
                          newSets[idx].price = Number(e.target.value);
                          setFormData({ ...formData, setOptions: newSets });
                        }} className="w-full text-sm border border-gray-300 rounded px-2 py-1.5" />
                      </div>
                      <div className="flex-1 min-w-[120px]">
                        <label className="block text-xs text-gray-500 mb-1">Compare-at (₹)</label>
                        <input type="number" value={setOpt.compareAtPrice || ''} onChange={(e) => {
                          const newSets = [...(formData.setOptions || [])];
                          newSets[idx].compareAtPrice = Number(e.target.value) || undefined;
                          setFormData({ ...formData, setOptions: newSets });
                        }} className="w-full text-sm border border-gray-300 rounded px-2 py-1.5" />
                      </div>
                      <button type="button" onClick={() => {
                        const newSets = (formData.setOptions || []).filter((_, i) => i !== idx);
                        setFormData({ ...formData, setOptions: newSets });
                      }} className="p-2 text-red-500 hover:bg-red-50 rounded shrink-0 mb-0.5"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {formData.sellingMode === 'moq' && (
              <div className="space-y-4 border p-4 rounded-lg bg-gray-50">
                <h4 className="text-sm font-semibold text-gray-800">MOQ Configuration</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white p-4 rounded border">
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Minimum Order Qty (MOQ)</label>
                    <input type="number" value={formData.moqConfig?.minimumQuantity || 5} onChange={(e) => setFormData({ ...formData, moqConfig: { ...(formData.moqConfig || { basePrice: 0, additionalUnitPrice: 0, maximumQuantity: undefined }), minimumQuantity: Number(e.target.value) } })} className="w-full text-sm border border-gray-300 rounded px-2 py-1.5" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Maximum Order Qty (Optional)</label>
                    <input type="number" value={formData.moqConfig?.maximumQuantity || ''} onChange={(e) => setFormData({ ...formData, moqConfig: { ...(formData.moqConfig || { basePrice: 0, additionalUnitPrice: 0, minimumQuantity: 5 }), maximumQuantity: Number(e.target.value) || undefined } })} className="w-full text-sm border border-gray-300 rounded px-2 py-1.5" placeholder="No limit" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Base Price (for MOQ units) ₹</label>
                    <input type="number" value={formData.moqConfig?.basePrice || 0} onChange={(e) => setFormData({ ...formData, moqConfig: { ...(formData.moqConfig || { minimumQuantity: 5, additionalUnitPrice: 0 }), basePrice: Number(e.target.value) } })} className="w-full text-sm border border-gray-300 rounded px-2 py-1.5" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Per-Additional Unit Price ₹</label>
                    <input type="number" value={formData.moqConfig?.additionalUnitPrice || 0} onChange={(e) => setFormData({ ...formData, moqConfig: { ...(formData.moqConfig || { minimumQuantity: 5, basePrice: 0 }), additionalUnitPrice: Number(e.target.value) } })} className="w-full text-sm border border-gray-300 rounded px-2 py-1.5" />
                  </div>
                </div>
              </div>
            )}
