      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm flex items-center">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">₹{data.summary.totalRevenueYTD}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm flex items-center">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-gray-900">₹{data.summary.averageOrderValue}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm flex items-center">
            ...
            <tbody className="divide-y divide-gray-200">
              {data.revenueByMonth.map((item, idx) => (
                <tr key={idx} className="bg-white hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right">₹{item.revenue}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            {data.topCategories.map((cat, idx) => (
              <div key={idx}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-gray-500">₹{cat.sales} ({cat.percentage}%)</span>
                </div>
