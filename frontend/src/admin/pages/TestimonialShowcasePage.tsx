...
import { getAdminTestimonialShowcases, createAdminTestimonialShowcase, deleteAdminTestimonialShowcase } from '../services/adminTestimonialShowcaseService';
...
export default function TestimonialShowcasePage() {
  ...
  const handleCreateShowcase = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mediaUrl = uploadData.data[0].secure_url;
      const mediaType = uploadData.data[0].type;
      ...
      {createModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 md:p-8 w-full max-w-lg shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-gray-900">Add Manual Showcase</h3>
            ...
            <form onSubmit={handleCreateShowcase} className="flex flex-col gap-5">
                ...
                {!file ? (
                  <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 rounded-2xl cursor-pointer hover:bg-gray-50 hover:border-black transition-colors group">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                      ...
                      <p className="text-xs text-gray-500">SVG, PNG, JPG, MP4 or WEBM</p>
                    </div>
                    <input required type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/*,video/mp4,video/webm,video/quicktime" />
                  </label>
                ) : (
              ...
              <div className="flex gap-3 mt-4 pt-4 border-t border-gray-100">
                <button type="button" onClick={() => setCreateModalOpen(false)} disabled={isUploading} className="flex-1 px-4 py-3 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors">
                  Cancel
                </button>
                <button type="submit" disabled={isUploading || !file} className={`flex-1 px-4 py-3 text-sm font-medium bg-black text-white rounded-xl hover:bg-gray-900 transition-all ${isUploading ? 'opacity-70 cursor-not-allowed' : 'shadow-lg hover:shadow-xl hover:-translate-y-0.5'}`}>
                  {isUploading ? (
