...
router.post('/', upload.array('media', 3), async (req, res) => {
  try {
    ...
    if (!category || !productSlug) {
      return res.status(400).json({ message: 'Category and productSlug are required to organize media.' });
    }
    ...
    const uploadPromises = req.files.map(file => {
      if (file.mimetype.startsWith('video/')) {
        return mediaService.uploadVideo(file.buffer, folderPath);
      } else {
        return mediaService.uploadImage(file.buffer, folderPath);
      }
    });
